...
un deuxième temps, nous avions à rendre le programme fonctionnel,
notamment par la création du système
...
\par Pour cette deuxième soutenance, nous devions donc finir la conception du parsuer et lexeur mais aussi réaliser en partie la gestion des requêtes.
...
de reconnaître les motifs de la grammaire dans une requête par
...
lui-même se fait sur deux axes majeurs.\\

\large \textbf{Parseur \& Lexeur}\\ \normalsize \normalfont

\par Initialement prévue sur les trois premières soutenances, la
...
comme achevé. Toutes les requêtes prévues ou presque sont donc
...
\par Les requêtes nommées ci-dessus sont reconnues et fonctionnent.

\subsection{Avancement Général du Site}

\par Quant au site, il est quasiment terminé. En effet,
...
\par Bref, vous l'aurez compris, pour cette deuxième soutenance notre objectif était de rendre le site Web du projet opérationnel à 100\%.\\
...
\par Le seul problème réel que nous ayons rencontré était la
...
plupart du temps, de réelles questions se posent.\\
\par Cependant, avec les précédentes recherches que nous avions
entreprises, ce problème a pu être contourné et le développement du projet a suivi son cours normal.
...
\begin{enumerate}
\item {\bf Première soutenance + Rappels :}
\newline
\begin{itemize}
...
pr\'esenté MySql, historique, et surtout le syst\`eme de fichiers
sur lequel on s'est bas\'e pour notre projet. D'autre part,
j'avais commenc\'e à coder quelques fonctions utiles, pour ma
deuxi\`eme soutenance . Pour cette soutenance, mon but, c'est de
m'occuper de tous ce qui est manipulation de fichiers, à savoir,
...
\par
Comme nous l'avons pr\'esenté dans la premi\`ere soutenance, notre
...
\end{verbatim}
\par
De cette façon on a un système de fichiers bien structuré, et très
facile à exploiter. Pour plus de détails voir section Julien
Assémat.
\newline
\end{itemize}
...
Dans cette partie je vais présenter toutes les nouvelles fonctions
que j'ai ajoutées au moteur epiEngine , ainsi que les liaisons
...
Cette fonction permet d'ajouter une donnée, dans une table, et
ainsi la création d'un fichier .data ou seront stockées les
données. \item[-] Exemple de code :
...
De cette maniere, on a defini à la fois la commande INSERT, et la
fonction qui va avec, et qui créé en même temps, notre
fichier.data, ou l'on stockera nos données. La commande est
\begin{verbatim}
INSERT INTO ma_table VALUES(donée1, donnée2).
\end{verbatim}
\item[-] {\bf Fonction  DELETE :} La fonction DELETE permet de
supprimer une donnée dans une table passée en paramètre, il s'agit
donc de suppression de données, alors on se place dans notre
fichier.data, et on cherche la donnée, et on la supprime, une fois
...
\end{verbatim}
Chaque ligne  a son prédicat, si il est à vrai, la ligne sera
supprimée .
...
lexeur, le système de fichiers et les fonctions principales
INSERT, DELETE, UPDATE qui vont avec, donc tout va bien pour le
moment, on est toujours en avance, on espère garder ce rythme
jusqu'à la fin de l'année, même si on a perdu un 4ème membre, à 3
on peut dire qu'on s'en sort plutôt bien.
...
\subsection{Tâches réaplisées par Thomas}
...
\par Il faut savoir que les données relatives à une table
...
projet de devenir réellement opérationnel.
